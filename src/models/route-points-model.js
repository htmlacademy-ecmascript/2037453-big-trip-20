...
export default class RoutePointsModel extends Observable {
  ...
  constructor({routePointsApiService}) {
    ...
    // this.#routePointsApiService.routePoints.then((resp) => {
    //   const adaptedResp = resp.map((routePoint) => this.#adaptToClient(routePoint));
    //   console.log(adaptedResp);
    // });
  }
  ...
  async init() {
    try {
      ...
      this.#routePoints = routePoints.map((routePoint) => this.#adaptToClient(routePoint));
    } catch(err) {
      this.#routePoints = [];
    }
